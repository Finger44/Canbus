...
char* user_data = "Initialization Complete!! \n\r";
char* user_data2 = "Warning error occur!! \n\r";
char* CanTansmit = "Can Data Transmitted!! \n\r";
uint32_t data_to_store[10]={0};
...
osThreadId TaskUsartMsgHandle;
osMessageQId MainScreenHandle;
osSemaphoreId BinarySemUsartMsgHandle;
...
void TaskusartMsg(void const * argument);

/* USER CODE BEGIN PFP */

void CAN1_Tx();
//void CAN_Filter_Config(void);//new
...
int main(void)
{
  ...
  if(HAL_CAN_ActivateNotification(&hcan2,CAN_IT_TX_MAILBOX_EMPTY |CAN_IT_BUSOFF) != HAL_OK)
  	{
  		Error_Handler();
  	}
  if(HAL_CAN_ActivateNotification(&hcan2,CAN_IT_RX_FIFO1_MSG_PENDING) != HAL_OK)
  	...
  	Flash_Read_Data(0x081E0000, data_to_store,10);
  ...
  /* definition and creation of BinarySemUsartMsg */
  osSemaphoreDef(BinarySemUsartMsg);
  BinarySemUsartMsgHandle = osSemaphoreCreate(osSemaphore(BinarySemUsartMsg), 1);
  ...
  /* Create the queue(s) */
  /* definition and creation of MainScreen */
  osMessageQDef(MainScreen, 16, uint16_t);
  MainScreenHandle = osMessageCreate(osMessageQ(MainScreen), NULL);
  ...
  /* definition and creation of TaskUsartMsg */
  osThreadDef(TaskUsartMsg, TaskusartMsg, osPriorityIdle, 0, 128);
  TaskUsartMsgHandle = osThreadCreate(osThread(TaskUsartMsg), NULL);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(io_GPIO_Port, io_Pin, GPIO_PIN_SET);
...
/*void CAN1_Tx(void)
{
	...
	uint8_t our_message[8] = {0X20, 0X6, 0XA3, 0X99, 0X03, 0x00, 0X8, 0X01 };

	TxHeader.DLC = 8;
	TxHeader.ExtId = 0x1def0522; //remote id
	TxHeader.IDE   = CAN_ID_EXT;
	TxHeader.RTR = CAN_RTR_DATA;

	if( HAL_CAN_AddTxMessage(&hcan2,&TxHeader,our_message,&TxMailbox) != HAL_OK)
	{
		Error_Handler();
	}

	else HAL_UART_Transmit(&huart3, (uint8_t*)CanTansmit, strlen(CanTansmit), HAL_MAX_DELAY);
...
/* USER CODE BEGIN Header_TaskusartMsg */
/**
* @brief Function implementing the TaskUsartMsg thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_TaskusartMsg */
void TaskusartMsg(void const * argument)
{
  /* USER CODE BEGIN TaskusartMsg */
	PollingInit();
  /* Infinite loop */
  for(;;)
  {
